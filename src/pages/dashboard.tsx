...
import { getLoveCalculations } from 'apiSdk/love-calculations';
import { LoveCalculationsInterface } from 'interfaces/love-calculations';
import { getDeviceInfos } from 'apiSdk/device-infos';
import { DeviceInfoInterface } from 'interfaces/device-info';
import { getErrorLogs } from 'apiSdk/error-logs';
import { ErrorLogsInterface } from 'interfaces/error-logs';

import { ShareListPage as ShowCaseList } from 'pages/shares';

function DashboardPage() {
  const { session } = useSession();

  const loveCalculationsStat = useSWR<PaginatedInterface<LoveCalculationsInterface>>(
    () => `/love-calculations`,
    () => getLoveCalculations({ limit: 1, offset: 0 }),
  );

  const deviceInfoStat = useSWR<PaginatedInterface<DeviceInfoInterface>>(
    () => `/device-infos`,
    () => getDeviceInfos({ limit: 1, offset: 0 }),
  );

  const errorLogsStat = useSWR<PaginatedInterface<ErrorLogsInterface>>(
    () => `/error-logs`,
    () => getErrorLogs({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Love Calculations',
      result: loveCalculationsStat?.data?.totalCount,
      isLoading: loveCalculationsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Device Infos',
      result: deviceInfoStat?.data?.totalCount,
      isLoading: deviceInfoStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Error Logs',
      result: errorLogsStat?.data?.totalCount,
      isLoading: errorLogsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`shares`} passHref legacyBehavior>
                  <Button
